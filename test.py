...
def train_kmeans(train_data, mode):
...
def plot_speedup(n_vals, seq_times, par_times):
    ...
    """
    speedup = [par_time / seq_time for seq_time, par_time in zip(seq_times, par_times)]
    fig, ax = plt.subplots()
...
def test_samples():
    ...
    for n in n_vals:
        seq_times.append(train_kmeans(train_data[:n].copy(), 'sequential'))
        par_times.append(train_kmeans(train_data[:n].copy(), 'parallel'))
...
def test_features():
    ...
    seq_times = []
    par_times = []
    n_features_vals = tuple(range(2, 51, 5))
    
    for n_features in n_features_vals:
        train_data = generate_data(n_samples=100000, n_features=n_features, n_clusters=5)
        seq_times.append(train_kmeans(train_data, 'sequential'))
        par_times.append(train_kmeans(train_data, 'parallel'))
...
def test_clusters():
    ...
    seq_times = []
    par_times = []
    centers_vals = tuple(range(2, 51, 5))
    
    for centers in centers_vals:
        train_data = generate_data(n_samples=100000, n_features=2, n_clusters=centers)
        seq_times.append(train_kmeans(train_data, 'sequential'))
        par_times.append(train_kmeans(train_data, 'parallel'))
